...
def agregar_producto():
    ...
    try:
        ...
        # DEF-001: Validar que cantidad y precio sean positivos
        if cantidad <= 0:
            messagebox.showerror("Error de Validación", "La cantidad debe ser un número entero mayor a cero.")
            return
        if precio <= 0:
            messagebox.showerror("Error de Validación", "El precio debe ser un número mayor a cero.")
            return

        # Validar que el nombre no sea exclusivamente numérico
        if nombre.isdigit():
            messagebox.showerror("Error de Validación", "El nombre del producto no puede ser solo números.")
            return

        # DEF-002: Verificar código de barras duplicado
        if sede_actual_id and sede_actual_id.get():
            inventario_actual = api_get(f"/inventario/{sede_actual_id.get()}")
            if inventario_actual:
                codigos_existentes = [str(p.get('codigo_barras', '')) for p in inventario_actual]
                if codigo in codigos_existentes:
                    messagebox.showerror("Código Duplicado",
                                         f"El código de barras '{codigo}' ya existe en el inventario.\n"
        ...
        if sede_actual_id and sede_actual_id.get():
            ...
            if inventario_actual:
                ...
                for p in inventario_actual:
                    if str(p.get('codigo_barras', '')) == codigo and p.get('id') != id_actual:
                        messagebox.showerror("Código Duplicado",
                                             f"El código de barras '{codigo}' ya pertenece a otro producto.\n"
                                             "Por favor utiliza un código diferente.")
                        return
...
def eliminar_producto():
    ...
    try:
        ...
        if messagebox.askyesno("Confirmar", "¿Estás seguro de borrar este producto?"):
            try:
                res = api_post(f"/inventario/eliminar/{sel[0]}", {}, metodo="DELETE")
                if res is not None:
                    messagebox.showinfo("Éxito", "Producto eliminado")
                else:
                    messagebox.showerror("Error", "No se pudo eliminar el producto. Verifica la conexión.")
                cargar_inventario()
            except Exception as e:
    ...
    try:
        ...
        if res and "id" in res:
            if res.get('cantidad', 0) > 0:
                # DEF-004: Preguntar cantidad, igual que en adición manual
                stock_disponible = res.get('cantidad', 0)
                cant = simpledialog.askinteger(
                    "Cantidad",
                    f"¿Cuántas unidades de '{res.get('nombre', '')}'?\n(Stock disponible: {stock_disponible})",
                    minvalue=1,
...
def guardar_deudor():
    ...
    try:
        ...
        # DEF-005: Validar dirección obligatoria
        direccion = entry_d_dir.get().strip()
        if not direccion:
            messagebox.showerror("Error de Validación", "La dirección es obligatoria.")
            return
        ...
        # Validar que deuda y crédito no sean negativos
        if deuda < 0:
            messagebox.showerror("Error de Validación", "La deuda inicial no puede ser negativa.")
            return
        if credito < 0:
            messagebox.showerror("Error de Validación", "El crédito máximo no puede ser negativo.")
            return

        # Validar día de visita
        if not combo_d_dia.get():
            messagebox.showerror("Error de Validación", "Selecciona un día de visita.")
            return

        # DEF-005: Verificar duplicado por nombre
        if sede_actual_id and sede_actual_id.get():
            deudores_existentes = api_get(f"/deudores/{sede_actual_id.get()}")
            if deudores_existentes:
                nombres_existentes = [d.get('nombre', '').strip().lower() for d in deudores_existentes]
                if nombre.lower() in nombres_existentes:
                    messagebox.showerror("Cliente Duplicado",
                                         f"Ya existe un cliente con el nombre '{nombre}'.\n"
                                         "Por favor verifica si ya está registrado.")
                    return
...
def abonar():
    ...
    try:
        ...
        # DEF-006: Verificar que el abono no supere la deuda actual
        did = int(sel[0])
        if did in DEUDORES_CACHE:
            deuda_actual = DEUDORES_CACHE[did].get('deuda', 0)
            if abono > deuda_actual:
                messagebox.showerror(
                    "Abono Excedente",
                    f"El monto del abono (${abono:.2f}) supera la deuda actual del cliente (${deuda_actual:.2f}).\n"
                    "Por favor ingresa un monto menor o igual a la deuda."
                )
                return
        ...
        def try_login():
            ...
            try:
                ...
                if res and "mensaje" in res and res["mensaje"] == "Login exitoso":
                    ...
                    # La API respondió None: problema de red o servidor caído
                    btn_login.config(text="INICIAR SESIÓN", state="normal")
                    messagebox.showerror("Error de Conexión",
